...
export function useTasks(filters = {}) {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      let query = supabase
        .from('tasks')
        .select(`*, categories(id, name), task_tags(id, tag_name)`)
  ...
  }, [user, filters.status, filters.category_id, filters.search, filters.overdue])
  ...
  const createTask = async ({ tags = [], ...taskData }) => {
    ...
    if (tags.length > 0) {
      const tagRows = tags.map(tag_name => ({ task_id: task.id, tag_name }))
  ...
  const updateTask = async (id, { tags, ...updates }) => {
    ...
    if (tags !== undefined) {
      ...
      if (tags.length > 0) {
        const tagRows = tags.map(tag_name => ({ task_id: id, tag_name }))
        const { error: tagError } = await supabase.from('task_tags').insert(tagRows)
        if (tagError) throw tagError
      }
    }
